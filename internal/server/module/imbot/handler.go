...
// SettingsStore returns the ImBot settings store, for lookups (e.g. resolving
// a bot's chat-id lock) outside the handler's own request paths.
func (h *Handler) SettingsStore() *db.ImBotSettingsStore {
	return h.store
}
