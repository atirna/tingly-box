...
// ProviderUsage.PctLimit(), Kind == WindowKindLimit only) — 0-100, already
// the "how much is used" answer, no further unit conversion.
//
// PctLimit, not Pct: a standing balance/credit (Kind == WindowKindResource
// — OpenRouter's key balance, Kimi Code's booster wallet, KimiK2's
// credits) needs a manual top-up rather than time to recover, so it must
// not drive an automatic "avoid this pool" decision the same way a
// self-healing allowance does — see .design/quota-semantics.md §8.1.
//
