...
func ConvertResponsesToOpenAIChat(rs *responses.Response, responseModel string) wire.ChatCompletionWire {
	...
	totalInputTokens := normalizedUsage.PromptTotalTokens()
	...
	if normalizedUsage.CacheReadTokens > 0 || normalizedUsage.CacheWriteTokens > 0 {
		usage.PromptTokensDetails = &wire.ChatCompletionPromptDetailsWire{
			CachedTokens:     int64(normalizedUsage.CacheReadTokens),
			CacheWriteTokens: int64(normalizedUsage.CacheWriteTokens),
		}
	}
