...
func ConvertChatToResponsesWire(resp *openai.ChatCompletion, responseModel, actualModel string) wire.ResponsesWireResponse {
	...
	usageWire := tokenUsageToResponsesWire(usage.FromOpenAIChatCompletion(resp.Usage))
...
func ConvertAnthropicBetaToResponsesWire(resp *anthropic.BetaMessage, responseModel, actualModel string) wire.ResponsesWireResponse {
	...
	usageWire := tokenUsageToResponsesWire(usage.FromAnthropicBetaMessage(resp.Usage))
...
func tokenUsageToResponsesWire(normalized *protocol.TokenUsage) *wire.ResponsesUsageWire {
	inputTokens := normalized.PromptTotalTokens()
	return &wire.ResponsesUsageWire{
		InputTokens:  int64(inputTokens),
		OutputTokens: int64(normalized.OutputTokens),
		TotalTokens:  int64(inputTokens + normalized.OutputTokens),
		InputTokensDetails: wire.ResponsesInputTokensDetailsWire{
			CachedTokens:     int64(normalized.CacheReadTokens),
			CacheWriteTokens: int64(normalized.CacheWriteTokens),
		},
		OutputTokensDetails: wire.ResponsesOutputTokensDetailsWire{
			ReasoningTokens: int64(normalized.ReasoningTokens),
		},
	}
}
