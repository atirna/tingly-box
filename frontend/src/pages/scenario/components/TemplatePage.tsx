...
const TemplatePage: React.FC<TemplatePageProps> = (props) => {
    ...
    if (!providers.length) {
        ...
        // First-run path: send users to Credentials with Connect AI already
        // open rather than the bare provider dialog — they get to browse the
        // catalog or paste a config snippet for auto-detection.
        return (
            <UnifiedCard size="full" title={title}>
                <EmptyState
                    ...
                    primaryAction={{
                        ...
                        onClick: onAddApiKeyClick || (() => navigate('/credentials?dialog=add')),
                    }}
                />
            </UnifiedCard>
        );
