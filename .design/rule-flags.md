...
| `thinking_effort` | enum (`""`/`off`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`) | reasoning | **yes** | override | 统一控制 extended thinking。映射为 Anthropic budget_tokens（minimal 1K / low 4K / medium 10K / high 20K / xhigh 24K / max 32K，每个 Claude 模型都认识这个字段）或 OpenAI reasoning_effort（六档全部原生支持）。空 = "By Client"（透传客户端参数）。**后续计划**：Claude 4.5+ 原生支持 `output_config.effort` 字段，但并非所有模型都认识它，安全下发需要按模型能力判断——这部分（含模型能力目录）作为后续 PR 单独引入，本 PR 只做阶梯扩展 + 沿用既有的 budget_tokens/reasoning_effort 机制。| `ThinkingModeTransform`（Type 1b，server-domain Transform）|
