...
| `salvage_truncated_stream` | bool | response | — | — | 上游流在吐出部分内容后被截断时，不再向客户端发 stream_error，而是合成规范收尾，保住已送达的部分内容。覆盖两类截断：**干净 EOF**（无 finish_reason / 终止事件即关流）与**传输层读错误**（上游直接掐断连接，unexpected EOF / connection reset）。两类**不**补救：in-band SSE 错误载荷（上游明确报错，掩盖即隐藏真实故障）与客户端主动取消。合成收尾按出口协议：Chat 出口补 `finish_reason:"stop"` chunk + `[DONE]`；Responses 出口补 `response.completed` 终止序列；Anthropic 出口补 `content_block_stop` + `message_delta`（stop_reason 按已流内容推导 end_turn/tool_use）+ `message_stop`。**无内容截断仍报错**——没有可保内容且保持可被 mid-request failover 重试。代价：客户端无法感知响应被截断，故默认关、按 rule 显式开启。覆盖路径：Chat→Chat passthrough、Chat→Responses、Chat→Anthropic(v1/beta)、Responses→Anthropic(v1/beta)；**未覆盖**：Responses→Responses passthrough（直通无事件累积，无法合成 completed 载荷）、Anthropic→Anthropic passthrough、非流式 assembly 路径（未写出任何字节，报错仍可被 failover 重试，比伪造完整响应更优）。| `hc.SalvageTruncatedStream`（Type 2：`typ.GetRuleFlags(ctx)` → HandleContext，注入点：`streamOpenAIChat` / `streamOpenAIChatToResponses` / `streamResponsesToAnthropic{,Beta}` / `StreamOpenAIChatToAnthropic{V1,Beta}WithMCP`）|
