...
| `salvage_truncated_stream` | bool | response | — | — | 上游流在吐出部分内容后被掐断（干净 EOF、无 finish_reason / 终止事件）时，不再向客户端发 stream_error，而是合成规范收尾（Chat 出口补 `finish_reason:"stop"` chunk + `[DONE]`；Responses 出口补 `response.completed` 终止序列），保住已送达的部分内容。**空流截断仍报错**——没有可保内容且保持 5xx 可被 mid-request failover 重试。代价：客户端无法感知响应被截断，故默认关、按 rule 显式开启（适用于会中途断流的不稳定上游，见 codex "stream closed before response.completed" 一案）。当前覆盖 Chat 上游两条出口路径（Chat→Chat passthrough、Chat→Responses 转换）；Responses/Anthropic 上游的截断守卫暂不受此 flag 影响。| `hc.SalvageTruncatedStream`（Type 2：`typ.GetRuleFlags(ctx)` → HandleContext，`streamOpenAIChat` / `streamOpenAIChatToResponses` 两处注入）|
